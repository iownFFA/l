...
let bots = [];
let maxBots = CLIENTS_MAX_BOTS; // Updated to use CLIENTS_MAX_BOTS
let wsServer = null;
...
const data = {
    ...
    clientVersion: 0,
    followMouse: true,
    party: "Salad", // Updated party name
    vshield: false
};

function centerText(text, totalWidth) {
    const padding = totalWidth - text.length;
    const padLeft = Math.floor(padding / 2); // Left padding for centering
    const padRight = padding - padLeft; // Right padding
    return ' '.repeat(padLeft) + text + ' '.repeat(padRight); // Construct centered text
}

// Function to show the start banner
function showStartBanner() {
    const boxWidth = 60; // Box width

    // Define the text for the box
    const title = `${BOLD}${CYAN}SaladBots${RESET}`;
    const developer = `${MAGENTA}Developed By Salad King${RESET}`;
    ...
    // Log each line centered without side borders
...
class Bot {
    constructor(gameIP, id) {
        ...
        this.proxyManager = new ProxyManager(); // Ensure ProxyManager is initialized
    ...
    onmessage(event) {
        ...
        try {
            if (this.decryptionKey) {
                ...
                // Handle the various message cases
...
// Function to disconnect bots slowly
